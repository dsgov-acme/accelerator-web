...
const dependencies = MockBuilder(DashboardComponent)
  ...
  .provide(
    MockProvider(DashboardService, {
      activeTransactions$: of({ items: transactions, pagingMetadata: new PagingResponseModel() }),

      loadActiveTransactions$: () => of({ items: transactions, pagingMetadata: new PagingResponseModel() }),
      loadPastTransactions$: () => of({ items: transactions, pagingMetadata: new PagingResponseModel() }),
      pastTransactions$: of({ items: transactions, pagingMetadata: new PagingResponseModel() }),
    }),
  )
  .build();
