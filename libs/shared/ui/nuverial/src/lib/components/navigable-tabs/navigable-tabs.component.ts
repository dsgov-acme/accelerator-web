...
export class NuverialNavigableTabsComponent implements OnInit {
  ...
  private _setTabIndex() {
    ...
    this.activeTabIndex = this.tabs.findIndex(tab => tab.route === currentRoute) !== -1 ? this.tabs.findIndex(tab => tab.route === currentRoute) : 0;
  ...
  public handleActiveTabChange(event: ActiveTabChangeEvent) {
    this._router.navigate([this.tabs[event.index].route], { relativeTo: this._route });
  }
}
