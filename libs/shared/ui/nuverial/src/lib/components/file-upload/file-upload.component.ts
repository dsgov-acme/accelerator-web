...
export class NuverialFileUploadComponent extends FormInputBaseDirective implements ControlValueAccessor, OnInit {
  ...
  public set fileStatus(updatedFileStatus: Map<string, FileStatus>) {
    ...
    for (const file of updatedFileStatus.values()) {
      ...
      updatedFileStatus.set(file.name, { ...file, progressWidth: `${file.uploadProgress}%`, status, statusMessage: this.getStatusMessage(status) });
    }
  ...
  public readonly singleFilePreview: BehaviorSubject<unknown> = new BehaviorSubject<unknown>('');
  ...
  private _setImagePreview(file: File): void {
    ...
    fr.onload = (e: ProgressEvent<FileReader>) => {
      this.singleFilePreview.next(e.target?.result);
      this._changeDetectorRef.markForCheck();
    };
  ...
  public dropHandler(event: DragEvent) {
    ...
    if (!files || this.isNullOrEmpty(files)) return;
  ...
  private isNullOrEmpty(fileList: FileList): boolean {
