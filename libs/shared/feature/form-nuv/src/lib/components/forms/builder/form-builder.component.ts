...
import { BehaviorSubject, catchError, EMPTY, finalize, forkJoin, Observable, of, Subject, Subscription, switchMap, take, takeUntil, tap } from 'rxjs';
...
export class FormBuilderComponent implements DoCheck, OnDestroy {
  ...
  public loadForm$ = this._route.paramMap.pipe(
    ...
    switchMap(({ transactionDefinition }) => {
      const schemaKey = transactionDefinition.schemaKey;

      return this._workApiRoutesService.getSchemaTree$(schemaKey);
    }),
    catchError(_error => {
