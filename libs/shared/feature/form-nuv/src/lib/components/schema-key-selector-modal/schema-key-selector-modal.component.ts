...
export class SchemaKeySelectorModalComponent implements OnInit {
  ...
  public loadSchemaTree$ = this._workApiRoutesService.schemaTree$.pipe(
    tap(schemaTree => {
      ...
      this.baseSchemaTree = this.disableDisallowedTypes(fullSchemaTree, this.allowedSchemaTypes);
      this.getSchemasList();
    }),
  );
  ...
  private isAllowedType(node: TreeNode, allowedTypes: Set<string>): boolean {
  ...
  private disableDisallowedTypes(root: TreeNode, allowedTypes: Set<string>): TreeNode {
    const disableDisallowedTypesRecursive = (current: TreeNode): TreeNode => {
      const allowed = this.isAllowedType(current, allowedTypes);
  ...
  public setSelectedSchemaKey(selectedNode: TreeNode): void {
    const fullSchemaKey = this.findFullSchemaKey(selectedNode, this.schemaTree);
