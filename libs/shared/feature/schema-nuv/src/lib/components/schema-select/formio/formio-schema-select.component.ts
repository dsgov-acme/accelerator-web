...
import { Filter } from '@dsg/shared/data-access/http';
...
export class FormioSchemaSelectComponent extends FormioBaseCustomComponent<ISchemaDefinition, AttributeBaseProperties> implements OnInit {
  ...
  private schemaKey = '';
  private readonly getSchemaKey$ = this._schemaBuilderService.metaDataFields$.pipe(tap(data => (this.schemaKey = data.key)));
  ...
  public ngOnInit(): void {
    this.getSchemaKey$.pipe(take(1)).subscribe();

    this._workApiRoutesService
      .getSchemaParents$(this.schemaKey)
      .pipe(
        take(1),
  ...
  private _selectSchema(value: string) {
    if (this.gotChildrenMap.get(value)) {
      ...
      if (selectedSchema) {
        this.triggerUpdateValue(selectedSchema);
      }
    }
  }
    ...
    return this._workApiRoutesService.getSchemaDefinitionsList$(filter).pipe(
      ...
      map(schemaDefinitions => {
        ...
        return schemaDefinitions
          .filter(schema => {
            const isCurrentSchema = schema.key === this.schemaKey;
  ...
  public handleSearchSchema(search: string) {
    if (search) {
      ...
      this._getSchemas$(filters).pipe(take(1)).subscribe();
    }
  }
      ...
      this._workApiRoutesService
        ...
        .pipe(
          tap((schema: ISchemaDefinition) => {
            ...
            this.triggerUpdateValue(schema);
          }),
          take(1),
        )
        .subscribe();
  ...
  private triggerUpdateValue(schema: ISchemaDefinition) {
