...
export class SchemaBuilderService {
  ...
  public schemaBuilderFormFields$: Observable<FormioForm> = this._updatedSchemaTree.asObservable().pipe(
    map(schemaTree => {
      const form = this.toSchemaBuilderForm(schemaTree);
  ...
  private toSchemaBuilderForm(schema: SchemaTreeDefinitionModel): FormioForm {
  ...
  public getSchemaTreeByKey$(schemaKey: string): Observable<SchemaTreeDefinitionModel> {
    ...
    return this.processSchemaTreeObservable(observable);
  }
  ...
  private processSchemaTreeObservable(_observable$: Observable<SchemaTreeDefinitionModel>): Observable<SchemaTreeDefinitionModel> {
  ...
  public updateFormComponents(formComponents: SchemaDefinitionModel): Observable<SchemaTreeDefinitionModel[]> {
    const schemaDefinitionTreeModel = formComponents;
    const apiCalls: Array<Observable<SchemaTreeDefinitionModel>> = [];

    schemaDefinitionTreeModel.attributes.forEach(attribute => {
