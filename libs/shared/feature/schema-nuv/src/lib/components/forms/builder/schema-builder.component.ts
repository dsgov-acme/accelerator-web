...
import { EMPTY, Observable, Subject, catchError, finalize, switchMap, take, takeUntil, tap } from 'rxjs';
...
export class SchemaBuilderComponent {
  ...
  public loadSchemaTree$ = this._route.paramMap.pipe(
    switchMap(params => {
      this.schemaKey = params.get('schemaKey') ?? '';

      return this._schemaBuilderService.getSchemaTreeByKey$(this.schemaKey);
    }),
    catchError(error => {
    ...
    switch (event) {
      case Actions.Save:
        ...
        this._schemaBuilderService
          .updateSchemaDefinition(form, this.schemaKey)
          .pipe(
            tap(_ => {
