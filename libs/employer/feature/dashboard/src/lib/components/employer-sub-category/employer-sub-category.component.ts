...
export class EmployerSubCategoryComponent {
  ...
  public loadTransactionDetails$ = this._formTransactionService.transaction$.pipe(filter(transactionModel => !!transactionModel.id));
  ...
  public loadSubCategory$ = this._route.paramMap.pipe(
    switchMap(params => {
      this.subCategoryRoute = params.get('subCategory') ?? '';
