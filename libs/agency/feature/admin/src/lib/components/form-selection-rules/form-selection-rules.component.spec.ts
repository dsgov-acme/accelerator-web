...
describe('FormSelectionRulesComponent', () => {
  ...
  it('should move item in formSelectionRulesList and form controls on drop and call updateValueAndValidity', () => {
    ...
    const moveItemInArrayMock = jest.fn();
    jest.spyOn(dragDrop, 'moveItemInArray').mockImplementation(moveItemInArrayMock);
    ...
    expect(moveItemInArrayMock).toHaveBeenCalledWith(component.formSelectionRulesList, event.previousIndex, event.currentIndex);
    expect(moveItemInArrayMock).toHaveBeenCalledWith(formArray.controls, event.previousIndex, event.currentIndex);
