...
export class TransactionDetailService {
  ...
  public customerProvidedDocuments$: Observable<ICustomerProvidedDocumentGroup[]> = this._customerProvidedDocuments.asObservable().pipe(
    map(documents => {
      ...
      const sortedDocBucketKeys = this._formTransactionService.formConfiguration.findComponentsKeyInOrder(docBucketKeys);
      ...
      return sortedDocBucketKeys
        ...
        .map(mappedDoucments => mappedDoucments as ICustomerProvidedDocument[])
        .map(mappedDocuments => ({
  ...
  private async _mapDocumentForRendering(document: ICustomerProvidedDocument): Promise<ICustomerProvidedDocument> {
    const formElement = this._formTransactionService.formConfiguration.getComponentDataByKey(document.dataPath);
